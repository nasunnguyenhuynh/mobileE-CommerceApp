...
const ReviewShopScreen = ({ route }: { route: any }) => {
  const shopReview = route.params?.shopReview
  const shopRating = route.params?.shopRating
  ...
  const renderShopReview = (item: any) => {
  ...
  return (
    <>
        ...
        <View>
          ...
          <View style={styles.commentsRatingsTitleDetail}>
            ...
            <View style={styles.wrapTotalProductRating}>
              <Text style={{ fontSize: 10, color: colors.lightGray }}>(Total {shopReview.length} ratings)</Text>
            </View>
          </View>
        </View>
        <View>
      ...
      <ScrollView style={styles.containerCommentsRatings}>
        {shopReview.map((review: { review: any }) => renderShopReview(review))}
      </ScrollView>
    </>
  )
}

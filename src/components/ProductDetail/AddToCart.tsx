...
import { saveCartToStorage } from '../../redux/storage';
...
const AddToCart: React.FC<AddToCartProps> = ({ visible = true, toggleModal, id = 0, price = 0, colors = [], images = [] }) => {
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    const selectedProduct: Product = {
                      id,
                      color: selectedProductColor?.id || null,
                      quantity
                    };
                    dispatch(addProduct(selectedProduct)); // add to cartState (can be renewd after log out)
                    saveCartToStorage(selectedProduct, store.getState().user.info?.id) // cannot renewed
                    toggleModal();
                  }}
                >
